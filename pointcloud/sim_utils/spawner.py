...
NUM_OF_OBJECTS_CANDIDATE = [4, 5]
STANDUP_ORIENTATION = [np.cos(np.deg2rad(45)), -np.sin(np.deg2rad(45)), 0, 0]


class SceneBuilder:
    def __init__(self, catalog: AssetCatalog, sampler: PositionSampler):
    ...
    def spawn_random(self):
        num_objects = random.choice(NUM_OF_OBJECTS_CANDIDATE)
        names = self.catalog.random_names(num_objects)
        positions = self.sampler.sample(num_objects)
