...
from ..data.constants.scannet import CLASS_LABELS as SCANNET_CLASSES
...
class SegPointTrainer:
    def __init__(self,
        ...
        # self.tasks = cfg.train.tasks
        # self.task_types = cfg.task_type

        self.dataloaders = dict()

        for task in cfg.tasks:
            for task_type in cfg.task_type:
                self.dataloaders[task].update({
                    task_type: build_dataloaders(task, task_type, cfg)
                })
        ...
        # 3. Get number of total steps and build Optimizer and Scheduler
        # 3-1. Compute the total length of all the train dataloaders
        total_len = 0
        for task, values in self.dataloaders.items():
            for task_type, loaders in values.items():
                total_len += len(loaders['train'])

        # 3-2. Compute total steps
        total_steps = ceil(total_len / gradient_accumulation_steps) * cfg.train.epochs

        self.optimizer, self.scheduler = build_optim(cfg, optim_params, total_steps=total_steps)
        # ===================================================
        # TODO: Study how the accelerator works!!
        # ===================================================
    ...
    def load_checkpoints(self,
        ...
        if self.mode == 'train':
            ...
            elif cfg.pretrained_ckpt_path and os.path.exists(cfg.pretrained_ckpt_path):
    ...
    def evaluate(self,
        ...
        for batch_idx, split in enumerate(output_split):
            ...
            batch_valid_seg_output.append(torch.stack(valid_seg_output))
            batch_valid_seg_gt.append(torch.stack(valid_seg_gt))

        padded_seg_output, padding_mask = pad_sequences(valid_seg_output)
        padded_seg_gt, _ = pad_sequences(valid_seg_gt)

        batch_mIoU, IoU = compute_mIoU_binary_groups(padded_seg_output, padded_seg_gt, padding_mask)

        # Compute Accuracy
        accurate_pred = IoU > 0.5                   # (B, G')
        accuracy = accurate_pred.sum(dim=1)         # (B, )
        accuracy /= num_cats
    ...
    def val_step(self, epoch, loader):
        ...
        # TODO: Do I have to generate evaluators like LEO..??
        # Wouldn't it be better to just simply write some functions because SegPoint tasks & metrics are much much less than LEO?
        all_mIoU = 0
        all_acc = 0
        ...
        results = {'mIoU': all_mIoU, 'accuracy': accuracy}
    ...
    @torch.no_grad
    def test_step(self, epoch, loader):
