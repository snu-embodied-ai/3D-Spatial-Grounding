...
class PointcloudEncoder(nn.Module):
    def __init__(self, point_transformer, cfg):
        ...
        self.trans_dim = cfg["pc_feat_dim"] # 768
        self.embed_dim = cfg["embed_dim"] # 512
        self.group_size = cfg["group_size"] # 32
        self.num_group = cfg["num_group"] # 512
        # grouper
        self.group_divider = Group(num_group = self.num_group, group_size = self.group_size)
        # define the encoder
        self.encoder_dim =  cfg["pc_encoder_dim"] # 256
        ...
        self.patch_dropout = PatchDropout(cfg["patch_dropout"]) if cfg["patch_dropout"] > 0. else nn.Identity()
...
class RegionPointcloudEncoder(PointcloudEncoder):
    def __init__(self, point_transformer, cfg):
        super().__init__(point_transformer, cfg)

    def forward(self, data_dict):
        # 0. Load the batch data and mask them
        divided_xyz = data_dict["divided_xyz"]
        divided_feature = data_dict["divided_features"]
